...
pub fn run(mut cli: Cli) -> io::Result<()> {
    if let Some(pos) = cli.bpm_positional {
        cli.bpm = pos;
    }

    let (numerator, denominator) = match parse_signature(&cli.signature) {
        Ok(v) => v,
        Err(e) => return Err(io::Error::new(io::ErrorKind::InvalidInput, e)),
    };

    if let Some(Commands::Tap { amount }) = &cli.command
        && let Some(bpm) = tap_tempo_blocking(*amount, numerator, denominator)?
    {
        cli.bpm = bpm;
    }

    let ticks_per_beat = cli.subdivision.ticks_per_beat();
    println!(
...
fn run_loop(
    ...
    if let Some(Commands::Ramp { pattern }) = &cli.command
        && let Some(cfg) = parse_ramp_pattern(pattern, parse_duration_ms)
    {
    ...
    loop {
        ...
        let is_accent = playing && tick_in_beat == 0 && beat_in_bar == 1;
        terminal.draw(|frame| {
            render_ui(
                frame,
                UiState {
